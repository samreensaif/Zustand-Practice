...
import { sneakersData } from "@/constants/sneakersData"
...
export default function Sneakers() {
  ...
  const toggleWishlist = (item: any) => {
    if (isInWishlist(item.id, "sneaker")) {
      // Pass 'sneaker' as the product type
      removeFromWishlist(item.id, "sneaker")
    } else {
      addToWishlist({
        id: item.id,
        name: item.name,
        price: item.price,
        image: item.image,
        productType: "sneaker", // Add product type
      })
    }
  }
  ...
  return (
    <div className="p-4 mx-auto lg:max-w-6xl md:max-w-4xl">
      <h2 className="text-2xl sm:text-3xl font-bold text-slate-900 mb-6 sm:mb-8 underline">Premium Sneakers</h2>

      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
        {sneakersData.map((item, index: number) => {
          const isWishlisted = isInWishlist(item.id, "sneaker") // Pass 'sneaker' as the product type
